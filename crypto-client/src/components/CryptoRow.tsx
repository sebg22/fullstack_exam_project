...
const CryptoRow = ({ coin }: Props) => {
  ...
  return (
    <Tr onClick={() => navigate(`/coin/${coin.id}`)} cursor="pointer" _hover={{ bg: "gray.100" }}>
      ...
      <Td p={{ base: 2, xl: 6 }}>${coin.current_price.toLocaleString()}</Td>
      <Td p={{ base: 2, xl: 6 }}></Td>
      ...
      <Td p={{ base: 2, xl: 6 }}>{formatCurrencyCompact(coin.market_cap)}</Td>
      <Td p={{ base: 2, xl: 6 }}>{formatCurrencyCompact(coin.total_volume)}</Td>
      <Td p={{ base: 2, xl: 6 }}>{formatNumberCompact(coin.circulating_supply)}</Td>
      <Td p={{ base: 2, xl: 6 }}>
