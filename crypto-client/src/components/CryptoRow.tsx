...
const CryptoRow = ({ coin }: Props) => {
  ...
  return (
    <Tr onClick={() => navigate(`/coin/${coin.id}`)} cursor="pointer" _hover={{ bg: "gray.600" }}>
      <Td p={{ base: 2, xl: 6 }}>
        <HStack spacing={2}>
          {" "}
          {/* Flex container with spacing */}
          {/* Maybe make this svg clickable to favorite it */}
          <svg style={{ flexShrink: 0 }} xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="gray" className="bi bi-star" viewBox="0 0 16 16">
      ...
      <Td p={{ base: 2, xl: 6 }} color="text">${coin.current_price.toLocaleString()}</Td>
      <Td p={{ base: 2, xl: 6 }} color="text"></Td>
      ...
      <Td p={{ base: 2, xl: 6 }} color="text">{formatCurrencyCompact(coin.market_cap)}</Td>
      <Td p={{ base: 2, xl: 6 }} color="text">{formatCurrencyCompact(coin.total_volume)}</Td>
      <Td p={{ base: 2, xl: 6 }} color="text">{formatNumberCompact(coin.circulating_supply)}</Td>
